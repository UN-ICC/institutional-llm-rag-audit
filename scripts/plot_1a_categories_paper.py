"""Plot Layer 1-A four-way response category breakdown for the paper figure.

Same layout as plot_1a_categories.py but with per-benchmark judge selection:
- HarmBench:      Llama-3.3 secondary judge (CAIS errors on 47.5% of Apertus
                  responses due to context-length truncation on long outputs).
- JailbreakBench: official JBB judge (Llama-3-70B-Instruct).
- StrongREJECT:   official GPT-4o rubric (Llama-3.3 categorisation is identical).

Usage:
    python scripts/plot_1a_categories_paper.py \
...
CATS = ("Refused", "Complied", "Ambiguous", "Jailbroken")
COLORS = {
    "Jailbroken": "#b91c1c",
    "Refused":    "#15803d",
    "Complied":   "#a8b3c4",
...
# Per-benchmark judge selection for the paper figure.
PER_BENCHMARK = {
    "harmbench": {
        "scores":     "scores_harmbench.llama33.bak.jsonl",
        "categorized": "categorized_harmbench_l33.jsonl",
    },
    "jailbreakbench": {
        "scores":     "scores_jailbreakbench.jsonl",
        "categorized": "categorized_jailbreakbench.jsonl",
    },
    "strongreject": {
        "scores":     "scores_strongreject.jsonl",
        "categorized": "categorized_strongreject.jsonl",
    },
}


def recategorize(run_dir: Path, bench: str):
    cfg = PER_BENCHMARK[bench]
    scored = {r["id"]: r for r in (json.loads(l) for l in open(run_dir / cfg["scores"]))}
    cat = [json.loads(l) for l in open(run_dir / cfg["categorized"])]
    counts = Counter()
    for cr in cat:
